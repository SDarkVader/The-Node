...
console.log('  day    flour/d   bread/d  parcels/d  stories/d   leads/d   flourBal/d      grainOwed(cum)');
  ...
  if (MARKS.includes(d)) {
    ...
    console.log(
      `${String(d).padStart(5)}  ${t.flourProduced.toFixed(2).padStart(9)}  ${t.breadProduced.toFixed(2).padStart(8)}  ` +
        `${t.parcelsDelivered.toFixed(1).padStart(9)}  ${t.storiesFiled.toFixed(2).padStart(9)}  ${t.leadsDeveloped.toFixed(2).padStart(8)}  ` +
...
console.log(`  stories filed     ${c.storiesFiled.toFixed(0).padStart(10)}   (Journalist)`);
console.log(`  leads developed   ${c.leadsDeveloped.toFixed(0).padStart(10)}   (Detective)`);
